...
def step_beta(step=1):
    ...
    next_iter = int(blue_step + 1) % blue_noise.shape[2]
    fraction = blue_step - this_iter

    noise_this = blue_noise[:, :, this_iter] * (1 - fraction) + blue_noise[:, :, next_iter] * fraction
...
def sim(r, s):
    ...
    for i in range(10000000):
        ...
        if i == N:
            ...
            print(max(ft), min(ft))
            fx = torch.fft.fftfreq(N)
            fy = torch.fft.fftfreq(N)
