...
da, db = 0.2, 0.1
beta = 0.0 + (torch.rand((N, N), device=device) * 0.1 - 0.05)
...
a = torch.full((N, N), 0.0, device=device)
b = torch.full((N, N), 1.0, device=device)
b -= beta
...
for i in range(1000000):
    ...
    b += dt * (g(a, b, f_sweep, k_sweep, diff_rate, beta) + db * get_laplacian(b))
